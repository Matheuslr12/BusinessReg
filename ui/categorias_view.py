...
class CategoriasView(tk.Frame):
    ...
    def create_header(self, parent):
        ...
        tk.Label(
            header, text='📁 Categorias', font=('Segoe UI', 24, 'bold'),
    ...
    def create_toolbar(self, parent):
        ...
        self.create_button(
            toolbar, '+ Adicionar Categoria', self.open_add_dialog,
    ...
    def open_category_dialog(self, categoria=None):
        ...
        self.create_button(
            actions, 'Salvar',
            lambda: self.save_category(dialog, nome_var.get(), descricao_var.get(), categoria['id'] if editing else None),
            self.colors['accent'], self.colors['text_primary'],
            ('Segoe UI', 10, 'bold'), 18, 9
        ).pack(side='right', padx=(0, 10))
        dialog.bind('<Return>', lambda event: self.save_category(
            dialog, nome_var.get(), descricao_var.get(), categoria['id'] if editing else None
        ))
    ...
    def delete_category(self):
        ...
        confirmed = messagebox.askyesno(
            'Excluir categoria',
            f'Tem certeza que deseja excluir "{categoria["nome"]}"?\n\nEssa ação não poderá ser desfeita.',
