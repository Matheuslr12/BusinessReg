...
class EmpresasView(tk.Frame):
    ...
    def create_table(self, parent):
        ...
        self.tree = ttk.Treeview(
            table_frame, columns=('id', 'nome', 'localizacao'),
            show='headings', style='Empresas.Treeview', selectmode='browse'
        )
    ...
    def load_companies(self, search=''):
        ...
        for empresa in empresas:
            self.tree.insert(
                '', 'end', iid=str(empresa['id']),
                values=(empresa['id'], empresa['nome'], empresa['localizacao'] or '-')
            )
        if empresas:
    ...
    def open_company_dialog(self, empresa=None):
        ...
        self.create_button(
            actions, 'Salvar',
            lambda: self.save_company(dialog, nome_var.get(), localizacao_var.get(), empresa['id'] if editing else None),
            self.colors['accent'], self.colors['text_primary'],
            ('Segoe UI', 10, 'bold'), 18, 9
        ).pack(side='right', padx=(0, 10))

        dialog.bind('<Return>', lambda event: self.save_company(
            dialog, nome_var.get(), localizacao_var.get(), empresa['id'] if editing else None
        ))
